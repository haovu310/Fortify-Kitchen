...
export default function TestPricingPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">🧪 Test Pricing</h1>
        <p className="text-sm text-slate-500 mt-1">Kiểm tra tính năng tính giá với các tổ hợp đơn hàng</p>
      </div>

      {/* Add item form */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
        <h2 className="text-sm font-semibold text-slate-600 mb-3">Thêm món</h2>
        <div className="flex flex-wrap gap-2 items-end">
          <div className="flex-1 min-w-[200px]">
            <label className="block text-xs text-slate-500 mb-1">Chọn món</label>
            <select
          ...
          <div className="w-20">
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm text-center focus:outline-none focus:ring-2 focus:ring-brand-500/40 focus:border-brand-500"
            />
          </div>
          <button
            onClick={addLineItem}
      ...
      {lineItems.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden animate-fade-in">
          <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
            <h2 className="text-sm font-semibold text-slate-600">Các món đã chọn</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-left">
                  <th className="px-4 py-2 font-medium text-slate-500">Món</th>
                  <th className="px-4 py-2 font-medium text-slate-500 text-center">SL</th>
                  <th className="px-4 py-2 font-medium text-slate-500 text-right">Đơn giá</th>
                  <th className="px-4 py-2 font-medium text-slate-500 text-right">Thành tiền</th>
                  <th className="px-4 py-2 font-medium text-slate-500 text-center">1kg?</th>
                  <th className="px-4 py-2"></th>
                </tr>
              </thead>
              <tbody>
                {result.lineDetails.map((line, i) => (
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-smooth">
                    <td className="px-4 py-2 font-medium text-slate-700">
                      {PROTEIN_LABELS[line.protein]} {line.flavor} ({line.sizeGrams}g)
                    </td>
                    <td className="px-4 py-2 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => updateQty(i, lineItems[i].qty - 1)} className="w-6 h-6 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 border-0 cursor-pointer text-sm">−</button>
                        <span className="w-8 text-center">{line.qty}</span>
                        <button onClick={() => updateQty(i, lineItems[i].qty + 1)} className="w-6 h-6 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 border-0 cursor-pointer text-sm">+</button>
                      </div>
                    </td>
                    <td className="px-4 py-2 text-right text-slate-600">{formatVND(line.unitPrice)}</td>
      ...
      {/* Pricing breakdown */}
      {result && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-4 animate-fade-in">
          {/* Per-protein summary */}
          <div>
            <h3 className="text-sm font-semibold text-slate-600 mb-2">📊 Tổng gram theo loại protein</h3>
          ...
          <div className="border-t border-slate-100 pt-3 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-slate-500">Tạm tính (sau giảm giá protein):</span>
              <span className="font-medium text-slate-700">{formatVND(result.lineSubtotal)}</span>
            </div>
            {result.orderDiscountTier ? (
              ...
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Giảm đơn hàng:</span>
                <span className="text-slate-400">Không áp dụng (dưới 1.5M)</span>
              </div>
            )}
